...
export default function FarmerLogin() {
  ...
  const handleFarmerAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.includes('auth/user-not-found')) msg = "Farmer not registered.";
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white border border-slate-200 overflow-hidden">
        ...
        <div className="bg-emerald-600 p-8 text-center relative overflow-hidden">
          ...
          <p className="text-emerald-100 text-sm mt-1 relative z-10">Farmer Portal</p>
        </div>
          ...
          <div>
            ...
            <form onSubmit={handleFarmerAuth} className="space-y-4">
              ...
              <button 
                type="submit" 
                disabled={loading}
                className="w-full bg-emerald-600 text-white rounded-none py-3 flex items-center justify-center gap-2 hover:bg-emerald-700 disabled:opacity-70 mt-2 text-sm"
              >
                {loading ? (
